...
new(Name) ->
    ...
    ets:insert(?METER_READER_TABLE,
               {Name, #meter_reader{one = OneMin, five = FiveMin, fifteen = FifteenMin,
                             start_time = folsom_utils:now_epoch_micro()}}).

tick(Name) ->
    #meter_reader{one = OneMin, five = FiveMin, fifteen = FifteenMin} = Meter = get_value(Name),
...
mark(Name, Value) ->
    ...
    #meter_reader{count = Count, last_count = LastCount,
                  one = OneMin, five = FiveMin,
...
get_acceleration(Name) ->
    #meter_reader{one = OneMin, five = FiveMin, fifteen = FifteenMin} = get_value(Name),
    [
